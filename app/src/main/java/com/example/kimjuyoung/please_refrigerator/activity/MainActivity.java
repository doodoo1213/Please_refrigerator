...
public class MainActivity extends AppCompatActivity implements View.OnClickListener{
    ...
    private void refresh_data(){
        ...
        vege_info.add(new Food_info("potato", "2018.01.02"));
        vege_info.add(new Food_info("onion", "2018.01.02"));
        vege_info.add(new Food_info("tomato", "2018.01.02"));
        vege_info.add(new Food_info("corn", "2018.01.02"));

        etc_info.add(new Food_info("egg", "2018.01.02"));
    }
    ...
    private void initList() {
        ...
        for (int i = 0; i < meat_info.size(); i++) {// 원하는 수만큼 버튼 생성
            ...
            btn_meats.elementAt(i).setText((CharSequence) meat_info.get(i).name);
        ...
        for (int i = 0; i < vege_info.size(); i++) {// 원하는 수만큼 버튼 생성
            ...
            btn_vegetables.elementAt(i).setText((CharSequence) vege_info.get(i).name);
        ...
        for (int i = 0; i < etc_info.size(); i++) {// 원하는 수만큼 버튼 생성
            ...
            btn_etcs.elementAt(i).setText((CharSequence) etc_info.get(i).name);
    ...
    public void onClick(View v){ //음식버튼의 동작

        int index = v.getId() % 100;//몇번째 버튼?

        switch(v.getId() / 100) { //category의 구분
